...
class JointRequestMapScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<JointRequestMapScreen> createState() =>
      _JointRequestMapScreen();
}

class _JointRequestMapScreen extends ConsumerState<JointRequestMapScreen> {
  bool firstLoad = true;
  ...
  void _setMarkersAllProjects(BuildContext context) {
    if (allRequests != null) {
      allRequests?.forEach((request) {
        if (request.requestLat != null) {
          _marker = Marker(
              markerId: MarkerId(request.requestId),
              position: LatLng(request.requestLat!, request.requestLng!),
              infoWindow: InfoWindow(
                  title: request.requestTitle, snippet: request.requestType),
              onTap: () {
                setState(() {
                  cardId = request.requestId;
                  ...
                  cardMaxPrice = request.requestMaxCost;
                });
              });
          _markers?.add(_marker!);
          _points?.add(LatLng(request.requestLat!, request.requestLng!));
        }
      });
    }
  }
